        ...
        private void button1_Click(object sender, EventArgs e)
        {
            try
            {
              ...
              log = string.Format("{0},{1},", user_name, pass);
                string sql = string.Format("select matk  from taikhoan where taikhoan = '{0}'", txttaikhoan.Text);
                if (Csdl_sys.check_val(cmd) == true)
                {
                    ...
                    QLKHOVATLIEU.frmMain.id_user_name = Class.Csdl_sys.ex_data_tring(sql);
